...
export const useConfirm = () => {
  ...
  const showConfirm = useCallback((message: string): Promise<boolean> => {
    return new Promise((resolve) => {
      setConfirmState({ isOpen: true, message, type: 'confirm', resolve });
    });
  }, []);

  const showAlert = useCallback((message: string): Promise<boolean> => {
    return new Promise((resolve) => {
      setConfirmState({ isOpen: true, message, type: 'alert', resolve });
    });
  }, []);
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-3xl w-full max-w-sm shadow-2xl animate-fade-in-up border border-gray-100 overflow-hidden">
          <div className="p-6">
            <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 ${confirmState.type === 'alert' ? 'bg-orange-100 text-orange-500' : 'bg-red-50 text-red-500'}`}>
              <AlertCircle size={24} />
            </div>
            <h3 className="text-xl font-bold font-serif text-gray-800 mb-2">
              {confirmState.type === 'alert' ? 'Aviso' : 'Atención'}
            </h3>
          ...
          <div className="flex bg-gray-50/80 p-4 gap-3 border-t border-gray-100">
            ...
            <button
              ...
              className={`flex-1 py-2.5 rounded-xl font-bold text-white transition-all text-sm shadow-md flex justify-center items-center gap-2 ${confirmState.type === 'alert' ? 'bg-brand-primary hover:bg-brand-accent hover:text-brand-primary' : 'bg-red-500 hover:bg-red-600'}`}
            >
